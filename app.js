document.getElementById('meal-button').addEventListener('click', () => {
    ...
    if (inputValue.value == '') {
        ...
        //fetch data
        fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${inputValue.value}`).then(res => res.json()).then(data => showMeals(data.meals)).catch(error => console.log(error));
        //clear search value
        inputValue.value = '';
    }
})
...
const loadDetails = mealId => fetch(`https://www.themealdb.com/api/json/v1/1/lookup.php?i=${mealId}`).then(res => res.json()).then(meal => showDetails(meal.meals[0]))
